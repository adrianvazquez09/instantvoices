...
class User extends Authenticatable
{
    ...
    /**
     * 
     * Added by adrian new slug logic
     * 1/may/21
     * 
     */

    function generateSlug($name,$role){
        $string='';
        if($role === 'employer'){
            $numeric = rand('0','9999999');
            $string = 'employer-'.$numeric;    
        } else if($role === 'freelancer'){
            $newname = explode(" ",$name);
            $numeric = rand('0','9999999');
            $string = 'voice-'.$numeric; 
        }
        return $string;
    }
    ...
    public function storeUser($request, $verification_code, $registration_type = '', $verification_type = '')
    {
        if (!empty($request)) {
            ...
            $this->slug = $this->generateSlug($this->first_name,$request['role']);
            ...
            if (!empty($request['employees'])) {//Modified by adrian 5/1/21
                $profile->no_of_employees = intval($request['employees']);
            }
            if (!empty($request['company_type'])) {
                $profile->company_type = filter_var($request['company_type'], FILTER_SANITIZE_STRING);
            }
            ...
            if (!empty($request['hourly_rate'])) {
                $profile->hourly_rate = floatval($request['hourly_rate']);
            }
            if (!empty($request['gender'])) {
                $profile->gender = filter_var($request['gender'], FILTER_SANITIZE_STRING);
            }
            if (!empty($request['exp_level'])) {
                $profile->exp_level = filter_var($request['exp_level'], FILTER_SANITIZE_STRING);
            }
            if (!empty($request['age_range'])) {
                $age_range = !empty($request['age_range']) ? serialize($request['age_range']): '';
                $profile->age_range = $age_range;
            }
            if (!empty($request['equipment'])) {
                $profile->equipment = filter_var($request['equipment'], FILTER_SANITIZE_STRING);
            }
